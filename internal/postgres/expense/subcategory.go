...
func (es *ExpenseSubCategoryRepo) InsertExpenseSubCategory(ctx context.Context, expenseSubCategory models.ExpenseSubCategory) (int64, error) {
...
func (es *ExpenseSubCategoryRepo) UpdateExpenseSubCategory(ctx context.Context, expenseSubCategory models.ExpenseSubCategory) (int64, error) {
	return 2, nil
}

func (es *ExpenseSubCategoryRepo) GetExpenseSubCategoryByID(ctx context.Context, id int64) (models.ExpenseSubCategory, error) {

	selectStmt := fmt.Sprintf("SELECT * FROM %s WHERE id = $1", tableNameSubCategories)

	row := es.database.QueryRowContext(ctx, selectStmt, id)

	var expenseSubCategory models.ExpenseSubCategory
	err := row.Scan(&expenseSubCategory.Id, &expenseSubCategory.Name, &expenseSubCategory.CategoryId)
	if err != nil {
		return models.ExpenseSubCategory{}, err
	}

	return expenseSubCategory, nil
}

func (es *ExpenseSubCategoryRepo) GetExpenseSubCategoryByName(ctx context.Context, name string) (models.ExpenseSubCategory, error) {
